...
# load coefficients
ctabfolder = 'B222ctabks/'
if wig:
	outputfolder = 'B222jmatbabis_cython/'
else:
	outputfolder = 'B222jmatbabisnowig_cython/'
...
def computeker(i1,i2,i3):
	...
	for i in range(len(ctab)):
		if ctab[i,3] != 0:
			term = ctab[i,3] * complex(Ltrian(ctab[i, 1] + fbabisparamtab[i1, 1], fbabisparamtab[i1, 2], ctab[i, 0] + fbabisparamtab[i2, 1],
...
def fill_with_conjugate(i1,i2,i3,res):
...
# filename = 'B222ctab_0.1_0.1_0.1.csv'
	...
	for i1 in reversed(range(lenfbabis)):
		for i2 in reversed(range(lenfbabis)):
			for i3 in reversed(range(lenfbabis)):
				print(Ltrian(ctab[i, 1] + fbabisparamtab[i1, 1], fbabisparamtab[i1, 2], ctab[i, 0] + fbabisparamtab[i2, 1],
												 fbabisparamtab[i2, 2], ctab[i, 2] + fbabisparamtab[i3, 1], fbabisparamtab[i3, 2], k12,
												 k22, k32, fbabisparamtab[i1, 0], fbabisparamtab[i2, 0], fbabisparamtab[i3, 0]))
				res = 0+0j
				for i in range(len(ctab)):

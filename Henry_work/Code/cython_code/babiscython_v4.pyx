...
mpfr_set_d(MPFR(CHOP_TOL), 1e-40, MPFRND)
	...
	for i in range(list_length):
		...
		if kqexp == 0:
			...
			if q2exp == 0:				
				...
				term = (numk2*delta_coef*k1dotk2(numk2,denk2,ksum2)/(sqrt(denk2)) + dkcoef*k1dotk2(numk2,denk2,ksum2)**3/(denk2*sqrt(denk2)))
				term += -m1*(numk2*delta_coef2*k1dotk2(numk2,denk2,ksum2)/(sqrt(denk2)) + dkcoef2*k1dotk2(numk2,denk2,ksum2)**3/(denk2*sqrt(denk2)))
			else:
				print('exceeded calculable power')
			...
			if kqexp%2 == 0:
				...
				if kqexp != 0:
					...
					# cases where kqexp == 6
						...
						#print(term, 'kqexp = 6, kqexp1 = 4, kqexp2 = 2')
...
# find remaining crossings of the imaginary axis
	...
	if almosteq(mpc(a), mpc(0), CHOP_TOL):
		if b != 0:
			xsol = [- c / b]
		else:
			xsol = []

...
# load coefficients
ctabfolder = 'simple_redshiftspace_bispectrum/B3212redctabks/'

outputfolder = 'simple_redshiftspace_bispectrum/B3212redjmatbabisnowig_cython/'
...
def compute_jmat(filename):
	...
	k2 = mpfr(str.split(filename,'_')[1])/10
	k3 = mpfr(str.split(filename,'_')[1])/10
	...
	for i1 in range(lenfbabis):
		...
		res = 0+0j
		for i in range(len(ctab)):
			term = ctab[i,-1] * Ltrian(-ctab[i, 1], 0, -ctab[i, 0] + fbabisparamtab[i1, 1],
									 fbabisparamtab[i1, 2], 0, 0, k12,
									 k22, mpfr(0), mpc(0), fbabisparamtab[i1, 0], mpc(0))

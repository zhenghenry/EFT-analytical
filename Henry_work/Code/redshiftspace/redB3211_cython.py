...
# load coefficients
ctabfolder = 'simple_redshiftspace_bispectrum/B3211redctabks/'

outputfolder = 'simple_redshiftspace_bispectrum/B3211redjmatbabisnowig_cython/'
...
def compute_jmat(filename):
	...
	ltriantable = np.zeros((len(fbabisparamtab),len(fbabisparamtab)),dtype = object)
	for i1 in reversed(range(lenfbabis)):
		for i2 in reversed(range(lenfbabis)):
			...
			for i in range(len(ctab)):
				if ctab[i,3] != 0:
					term = ctab[i,3] * Ltrian(-ctab[i, 1], 0, -ctab[i, 0] + fbabisparamtab[i1, 1],
											 fbabisparamtab[i1, 2], -ctab[i, 2] + fbabisparamtab[i2, 1], fbabisparamtab[i2, 2], k12,
											 k22, k32, mpc(0), fbabisparamtab[i1, 0], fbabisparamtab[i2, 0])
					# print('i',i,'term = ', "{0:.10f}".format(term))			
					res += term
			# print(type(res))
			ltriantable[i1,i2] = res

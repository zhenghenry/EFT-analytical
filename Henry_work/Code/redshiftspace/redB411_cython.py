...
# load coefficients
ctabfolder = 'simple_redshiftspace_bispectrum/B411redctabks/'

outputfolder = 'simple_redshiftspace_bispectrum/B411redjmatbabisnowig_cython/'
...
def compute_jmat(filename):
	...
	ctab = np.zeros((len(ctab_load),7), dtype = object)
	...
	for i in range(len(ctab)):
		...
		ctab[i, 6] = complex(str(ctab_load[i, 6]))
	...
	for i1 in range(lenfbabis):
		...
		for i in range(len(ctab)):
			...
			if ctab[i, 1] != 0:
				term = ctab[i,-1] * Ltrian(-ctab[i, 2], 0, -ctab[i, 0] + fbabisparamtab[i1, 1],
										 fbabisparamtab[i1, 2], -ctab[i, 4], 0, k12,
										 k22, k32, mpc(0), fbabisparamtab[i1, 0], mpc(0))
			# print('i',i,'term = ', "{0:.10f}".format(term))			
			if ctab[i, 3] != 0:
				term = ctab[i,-1] * Ltrian(-ctab[i, 2] + fbabisparamtab[i1, 1], fbabisparamtab[i1, 2], -ctab[i, 0],
										 0, -ctab[i, 4], 0, k12,
										 k22, k32, fbabisparamtab[i1, 0], mpc(0), mpc(0))
			if ctab[i, 5] != 0:
				term = ctab[i,-1] * Ltrian(-ctab[i, 2], 0, -ctab[i, 0],
										 0, -ctab[i, 4] + fbabisparamtab[i1, 1], fbabisparamtab[i1, 2], k12,
										 k22, k32, mpc(0), mpc(0),fbabisparamtab[i1, 0])
			
			res += term

		ltriantable[i1] = res

		# fill_with_conjugate(i1,i2,res,ltriantable)
...
def fill_with_conjugate(i1,i2,res,ltriantable):
	...
	if i1 > 0 and i2 > 0:
		if i1%2 == 0 and i2%2 == 0:
			if ltriantable[i1-1,i2-1] == 0:
				ltriantable[i1-1,i2-1] = resconj
	...
	if i1 == 0 and i2 > 0:
		if i2%2 == 0:
			if ltriantable[0,i2-1] == 0:
				ltriantable[0,i2-1] = resconj
		elif i2%2 == 1:
			if ltriantable[0,i2+1] == 0:
				ltriantable[0,i2+1] = resconj
	...
	if i1 > 0 and i2 == 0:
		if i1%2 == 0:
			if ltriantable[i1-1,0] == 0:
				ltriantable[i1-1,0] = resconj
		elif i1%2 == 1:
			if ltriantable[i1+1,0] == 0:
				ltriantable[i1+1,0] = resconj
	return 0
